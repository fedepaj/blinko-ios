...
xcrun devicectl device copy from --device "$D" --source Documents/rslog_diag.txt --destination "$HERE/build/rslog_diag.txt" --domain-type appDataContainer --domain-identifier com.federicopaglioni.rslogviewer >/dev/null 2>&1
ls -la "$OUT"
