...
mkdir -p "$OUT"
xcrun devicectl device copy from --device "$D" --source Documents/recordings --destination "$OUT" --domain-type appDataContainer --domain-identifier com.federicopaglioni.blinko 2>&1 | grep -iE "error" 
